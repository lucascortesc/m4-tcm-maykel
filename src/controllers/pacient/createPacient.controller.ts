...
export const createPacient = async (req: Request, res: Response) => {
  ...
  const { age, cpf, family_id, is_owner, last_name, name, tel }: ICreatePacient = req.body;

  const newPacient = await createPacientService({ age, cpf, family_id, is_owner, last_name, name, tel }, agentId);
