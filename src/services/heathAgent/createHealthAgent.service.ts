...
const createHealthAgentService = async (
  ...
  const emailData: IEmailRequest = {
    ...
    text: `<h1>Por favor confirme seu email<h1>
          <h3>Seja bem-vindo ao CIPAD ${name}, ative sua conta clicando no link: <a>https://password-recovery-cipad.vercel.app/activate/${activationToken}</a> para utilizar o nosso sistema</h3>
    `,
    to: email,
  };
